...
<div class="main-container container">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col">
                <h1 class="text-primary mr-auto">Vibhag & Yojana</h1>
            </div>
            <div class="col d-flex justify-content-end align-self-center">
                <a href="{{ route('vibhag.vibhag-yojana.create') }}" class="btn btn-outline-primary pull-right">Create New</a>
            </div>
        </div>
    </div>
    <!-- /heading -->
    ...
    <table class="table table-striped table-bordered data-table" cellspacing="0" width="100%">
        <thead class="thead-dark">
            <tr>
                <th>#</th>
                <th>Vibhag & Yojana Name</th>
                <th>Action</th>
            </tr>
        </thead>
        <tbody>
            @if($vibhagYojanaList->isNotEmpty())
            @foreach($vibhagYojanaList as $item)
            <tr>
                <td class="align-middle">{{ $loop->iteration }}</td>
                <td class="align-middle">{{ $item->name }}</td>
                <td class="align-middle">
                    <a href="{{ route('vibhag.vibhag-yojana.edit', $item->id) }}" class="btn btn-primary"><i class="fa fa-edit"></i> Edit</a>
                    <form method="POST" class="delete-data-form" action="{{ route('vibhag.vibhag-yojana.delete', $item->id) }}" hidden>
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                    </form>
