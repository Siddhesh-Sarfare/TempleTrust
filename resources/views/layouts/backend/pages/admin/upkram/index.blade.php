...
@section('page-content')
    <div class="main-container container">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col">
                    <h1 class="text-primary mr-auto">upkram</h1>
                </div>
                <div class="col d-flex justify-content-end align-self-center">
                    <a href="{{ route('admin.upkram.create') }}" class="btn btn-outline-primary pull-right">Create New</a>
                </div>
            </div>
        </div>
        <!-- /heading -->
        ...
        <table class="table table-striped table-bordered data-table" cellspacing="0" width="100%">
            ...
            <tbody>
                @if($upkramList->isNotEmpty())
                @foreach($upkramList as $item)
                <tr>
                    <td class="align-middle">{{ $loop->iteration }}</td>
                    <td class="align-middle">{{ $item->title }}</td>
                    <td class="align-middle">
                        <a href="{{ route('admin.upkram.edit', $item->id) }}" class="btn btn-primary"><i
                                class="fa fa-edit"></i> Edit</a>
                        <form method="POST" class="delete-data-form" action="{{ route('admin.upkram.delete', $item->id) }}"
                            hidden>
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                        </form>
